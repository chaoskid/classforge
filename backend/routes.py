...
@survey_routes.route('/api/relationship-distribution', methods=['GET'])
@either_login_required
def get_relationship_distribution():
    ...
    try:
        ...
        # Fetch all students in the teacher's unit
        student_ids = db.query(Allocations.student_id).filter_by(unit_id=unit_id).all()
        student_ids = [sid[0] for sid in student_ids]

        # Get relationship counts by link_type among students in that unit
        result = (
            db.query(Relationships.link_type, func.count().label('count'))
            .filter(Relationships.source.in_(student_ids), Relationships.target.in_(student_ids))
            .group_by(Relationships.link_type)
            .all()
        )

        response = [{"link_type": row.link_type, "count": row.count} for row in result]
